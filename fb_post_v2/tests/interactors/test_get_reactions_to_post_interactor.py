...
class TestGetReactionsToPost(unittest.TestCase):
    ...
    def test_get_reactions_to_post(self):
        ...
        post_id = 1
        response_data = [{"user_id": 1, "name": "user", "profile_pic_url": "url",
                          "reaction_type": "LOVE"}]

        reactions_dto = [UserReactionDTO(
            user_id=1, name="user", profile_pic_url="url", reaction_type="LOVE")]
        ...
        get_reactions_to_post_interactor = GetPostReactionsInteractor(
            post_storage_mock, presenter_mock)
        response = get_reactions_to_post_interactor.get_post_reactions(
            post_id, 0, 2)
