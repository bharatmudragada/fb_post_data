...
from fb_post_v2.interactors.get_positive_posts_interactor import \
...
class TestGetPositivePostsInteractor(unittest.TestCase):
    ...
    def test_positive_posts(self):
        ...
        post_storage_mock.get_posts_with_more_positive_reactions.return_value\
            = post_ids
        presenter_mock.get_posts_with_more_positive_reactions_response\
            .return_value = response_data

        get_positive_posts_interactor = \
            GetPostsWithMorePositiveReactionsInteractor(
                post_storage_mock, presenter_mock)
        response = get_positive_posts_interactor.get_positive_posts()

        post_storage_mock.get_posts_with_more_positive_reactions\
            .assert_called_once_with()
        presenter_mock.get_posts_with_more_positive_reactions_response\
            .assert_called_once_with(post_ids)
