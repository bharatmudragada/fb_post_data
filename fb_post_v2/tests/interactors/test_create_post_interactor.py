...
class TestCreatePost(unittest.TestCase):
    ...
    def test_create_post(self):
        ...
        post_dto = PostDTO(post_id=post_id, user_id=user_id, post_content=post_content, created_time=datetime.now())

        post_storage_mock.create_post.return_value = post_dto
        presenter_mock.get_create_post_response.return_value = response_data

        create_post_interactor = CreatePostInteractor(post_storage_mock, presenter_mock)
        response = create_post_interactor.create_post(post_content, user_id)

        post_storage_mock.create_post.assert_called_once_with(post_content, user_id)
