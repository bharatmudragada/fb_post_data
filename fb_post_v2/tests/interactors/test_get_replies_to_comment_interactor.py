...
from fb_post_v2.interactors.get_replies_to_comment_interactor import \
...
class TestGetRepliesToComment(unittest.TestCase):
    ...
    def test_get_replies_to_comment_with_comment(self):
        ...
        get_replies_to_comment_interactor = GetCommentRepliesInteractor(
            post_storage_mock, presenter_mock)
        response = get_replies_to_comment_interactor.get_comment_replies(
            comment_id, 0, 2)

        post_storage_mock.is_reply.assert_called_once_with(comment_id)
        post_storage_mock.get_comment_replies.assert_called_once_with(
            comment_id, 0, 2)
        presenter_mock.get_comment_replies_response.assert_called_once_with(
            replies_dto)
    ...
    def test_get_replies_to_comment_with_reply(self):
        ...
        with self.assertRaises(BadRequest):
            get_replies_to_comment_interactor.get_comment_replies(
                comment_id, 0, 2)
