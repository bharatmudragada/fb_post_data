...
class PostReactions(models.Model):
    ...
    reaction_type = models.CharField(
        max_length=10, choices=Reaction.get_reactions())

    class Meta:
        unique_together = ('post', 'user')

    def __str__(self):
        return str(self.post.post_content) + ", " +\
               str(self.user) + ", " + str(self.reaction_type)
...
class Comment(models.Model):
    ...
    def __str__(self):
        if not self.commented_on is None:
            return str(self.pk) + ", " + str(self.commented_on.pk) +\
                   ", " + str(self.user) + ", " + str(self.comment_text)
        else:
            return str(self.pk) + ", " + str(self.user) \
...
class CommentReactions(models.Model):
    ...
    def __str__(self):
        return str(self.comment.comment_text) + ", " + str(self.user) +\
               ", " + str(self.reaction_type)
